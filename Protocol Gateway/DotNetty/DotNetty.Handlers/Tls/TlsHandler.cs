...
namespace DotNetty.Handlers.Tls
{
    ...
    public sealed class TlsHandler : ByteToMessageDecoder
    {
        ...
        public static TlsHandler Client(
            string targetHost,
            X509Certificate2 certificate,
            RemoteCertificateValidationCallback certificateValidationCallback,
            SslProtocols sslProtocolsforClient = SslProtocols.Tls | SslProtocols.Tls11 | SslProtocols.Tls12)
        {
            return new TlsHandler(false, certificate, targetHost, certificateValidationCallback, sslProtocolsforClient);
        }
        ...
        bool EnsureAuthenticated()
        {
            ...
            if ((oldState & State.AuthenticationStarted) == 0)
            {
                ...
                if (this.isServer)
                {
                    this.sslStream.AuthenticateAsServerAsync(this.certificate, false, this.sslProtocols, false) // todo: change to begin/end
                ...
                else
                {
                    ...
                    this.sslStream.AuthenticateAsClientAsync(this.targetHost, certificateCollection, this.sslProtocols, false) // todo: change to begin/end
                        .ContinueWith(AuthenticationCompletionCallback, this, TaskContinuationOptions.ExecuteSynchronously);
                }
                return false;
            }
