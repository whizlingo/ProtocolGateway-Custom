...
namespace DotNetty.Codecs.Mqtt
{
    ...
    static class Signatures
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static bool IsConnect(int signature)
        {
            return (signature & TypeOnlyMask) == ((int)PacketType.CONNECT << 4);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static bool IsPubRel(int signature)
        {
            return (signature & TypeOnlyMask) == ((int)PacketType.PUBREL << 4);
        }
