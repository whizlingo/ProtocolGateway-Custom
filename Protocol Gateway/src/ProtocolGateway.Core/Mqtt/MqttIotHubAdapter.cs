...
namespace Microsoft.Azure.Devices.ProtocolGateway.Mqtt
{
    ...
    public sealed class MqttIotHubAdapter : ChannelHandlerAdapter
    {
        ...
        const string ClientIdKeyIdentifier = "vin";
        ...
        static readonly Action<Task> ShutdownOnPubRelFaultAction = CreateScopedFaultAction("-> PUBREL");
        ...
        readonly RequestAckPairProcessor<AckPendingMessageState, PubRecPacket> publishPubRelProcessor;
            ...
            ITopicNameRouter topicNameRouter, IQos2StatePersistenceProvider qos2StateProvider)
        {
            ...
            this.publishPubRelProcessor = new RequestAckPairProcessor<AckPendingMessageState, PubRecPacket>(this.AcknowledgePublishReleaseAsync, null, ackTimeout);
            this.publishPubRelProcessor.Completion.OnFault(ShutdownOnPubRelFaultAction);
        }

        // Should be called when Pub rel is recieved from the Device.
        private async Task AcknowledgePublishReleaseAsync(IChannelHandlerContext context, AckPendingMessageState message)
        {
            try
            {
                ...
                IQos2MessageDeliveryState messageInfo = await this.qos2StateProvider.GetMessageAsync(message.PacketId, message.ClientId);
                if (messageInfo != null)
                {
                    await this.qos2StateProvider.DeleteMessageAsync(message.PacketId, messageInfo);
                }

                Util.WriteMessageAsync(context, pubCompPacket)
        ...
        void ProcessMessage(IChannelHandlerContext context, Packet packet)
        {
            ...
            switch (packet.PacketType)
            {
                ...
                case PacketType.PUBREL:
                    this.publishPubRelProcessor.Post(context, (PubRelPacket)packet);
                    break;
            ...
            if (!this.IsInState(StateFlags.Closed))
            {
                switch (packet.QualityOfService)
                {
                    case QualityOfService.AtMostOnce:
                        ...
                        await CompleteTheMessageAndSendToIOTHub(packet);
                    ...
                    case QualityOfService.ExactlyOnce:
                        // Publish to server using QOS 2 handshake and send Pub Rec message.
                        await PublishToServerQos2Async(context, packet);
                        break;
                    default:
        ...
        private async Task CompleteTheMessageAndSendToIOTHub(PublishPacket packet)
        {
        ...
        private async Task PublishToServerQos2Async(IChannelHandlerContext context, PublishPacket packet)
        {
            //this.ResumeReadingIfNecessary(context);
            using (Stream bodyStream = packet.Payload.IsReadable() ? new ReadOnlyByteBufferStream(packet.Payload, true) : null)
            {
                ...
                MqttIotHubAdapterEventSource.Log.Info("Message Id recieved in the message is {0}", message.MessageId);
                // Get the message Info from QOS 2 persistence store.
                int packetId = packet.PacketId;
                IQos2MessageDeliveryState messageInfo = await this.qos2StateProvider.GetMessageAsync(packetId, deviceId);
                 //&& !message.MessageId.Equals(messageInfo.MessageId, StringComparison.Ordinal)
                //if (messageInfo != null)
                //{
                //    MqttIotHubAdapterEventSource.Log.Info("Message Id is different here", "info");
                //    await this.qos2StateProvider.DeleteMessageAsync(packetId, messageInfo);
                //    messageInfo = null;
                //}
                if (messageInfo == null)
                {
                    messageInfo = this.qos2StateProvider.Create(message.MessageId);
                    await this.qos2StateProvider.SetMessageAsync(packet.PacketId, deviceId, messageInfo);

                    // Send the message to IOT hub.
                    MqttIotHubAdapterEventSource.Log.Info("Send the message to IOT Hub", "info");

                    await this.iotHubClient.SendAsync(message);

                    await PublishPublishRecievedAsync(context, packet, message);
                }
                else
                {
                    await PublishPublishRecievedAsync(context, packet, message);
        ...
        Task PublishPublishRecievedAsync(IChannelHandlerContext context, PublishPacket packet, Message message)
        {
            var pubRecPacket = new PubRecPacket
            {
                PacketId = packet.PacketId
            };
            return this.publishPubRelProcessor.SendRequestAsync(context, pubRecPacket,
                new AckPendingMessageState(message, packet, deviceId));
        }
        ...
        async void Receive(IChannelHandlerContext context)
        {
            ...
            try
            {
                ...
                MqttIotHubAdapterEventSource.Log.Info("Processing the message id : {0}", message.MessageId);
        ...
        {
            MqttIotHubAdapterEventSource.Log.Info(string.Format("Publish the message with device id {0}",
                this.deviceId), "info");
        ...
        private async Task PublishToClientQos2Async(IChannelHandlerContext context, Message message,
            PublishPacket packet)
        {
            ...
            // Get the client Id.
            string clientId;
            message.Properties.TryGetValue(ClientIdKeyIdentifier, out clientId);
            ...
            IQos2MessageDeliveryState messageInfo = await this.qos2StateProvider.GetMessageAsync(packetId, clientId);
            ...
            if (messageInfo == null)
            {
                MqttIotHubAdapterEventSource.Log.Info(string.Format("Publish the message with message id {0} to device id {1}", message.MessageId, this.deviceId), "info");
                await this.publishPubRecProcessor.SendRequestAsync(context, packet, new AckPendingMessageState(message, packet, clientId));

            }
            else
            {
                MqttIotHubAdapterEventSource.Log.Info(string.Format("Publish release to client to device id {0}", this.deviceId), "info");
                await this.PublishReleaseToClientAsync(context, packetId, message.LockToken, messageInfo, PreciseTimeSpan.FromStart);
            }
        }

        Task PublishReleaseToClientAsync(IChannelHandlerContext context,
            int packetId,
            string lockToken,
            IQos2MessageDeliveryState messageState,
            PreciseTimeSpan startTimestamp)
        {
        ...
        async Task AcknowledgePublishReceiveAsync(IChannelHandlerContext context, AckPendingMessageState message)
        {
            ...
            try
            {
                ...
                await this.qos2StateProvider.SetMessageAsync(message.PacketId, message.ClientId, messageInfo);
        ...
        bool TryMatchSubscription(string topicName, DateTime messageTime, out QualityOfService qos)
        {
            ...
            foreach (Subscription subscription in this.sessionState.Subscriptions)
            {
                ...
                {
                    MqttIotHubAdapterEventSource.Log.Info(string.Format("Failed to find matching subscription out of session state " +
                                                          "having {0} Subscriptions", this.sessionState.Subscriptions.Count), "info");
                }
            }
            return found;
        }
        ...
        async void Connect(IChannelHandlerContext context, ConnectPacket packet)
        {
            ...
            try
            {
                ...
                await this.EstablishSessionStateAsync(this.deviceId, packet.CleanSession);
